...
public class Map {
	public int bricksXPos[] = {50,350,450,550,50,300,350,450,550,150,150,450,550,
						250,50,100,150,550,250,350,450,550,50,250,350,550,
						50,150,250,300,350,550,50,150,250,350,450,550,50,
						250,350,550};
	
	public int bricksYPos[] = {50,50,50,50,100,100,100,100,100,150,200,200,200,250,
	...
	public boolean checkCollision(int x, int y, int width, int height)
	{
		...
		for(int i=0; i< brickON.length;i++)
		{
			if(brickON[i]==1)
			{
				if(new Rectangle(x, y, width, height).intersects(new Rectangle(bricksXPos[i], bricksYPos[i], 50, 50)))
				{
					if(width != 50) brickON[i] = 0;
					collided = true;
					break;
				}
			}
	...
	public boolean checkSolidCollision(int x, int y, int width, int height)
	{
		...
		for(int i=0; i< solidBricksXPos.length;i++)
		{		
			if(new Rectangle(x, y, width, height).intersects(new Rectangle(solidBricksXPos[i], solidBricksYPos[i], 50, 50)))
			{		
				collided = true;
				break;
			}			
